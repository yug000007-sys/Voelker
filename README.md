...
## v7 ReferralEmail Fix

This version fills `ReferralEmail` from `ReferralManager` using `get_referral_email()`.

Privacy-safe setup for GitHub/Streamlit Cloud:
1. Do not commit `.streamlit/secrets.toml`. It is ignored by `.gitignore`.
2. In Streamlit Cloud, open App Settings > Secrets and paste:
...
Add the remaining manager emails there. The app checks Streamlit Secrets first.
